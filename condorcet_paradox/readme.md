...
Un module Python Orienté Objet (src/condorcet.py) permettant de représenter des électeurs, d'évaluer des duels par paires, d'exécuter l'algorithme DFS de détection de cycles et de lancer des simulations de Monte-Carlo.

Une suite de tests unitaires avec Pytest (tests/test_condorcet.py) garantissant la qualité et la robustesse des algorithmes.

Un Notebook Jupyter interactif (notebooks/condorcet_paradox_exploration.ipynb) pour expérimenter avec des cas concrets (politique, entreprise) et générer des cartes thermiques/graphiques.

Un article de fond exhaustif (article.md) détaillant la théorie mathématique, le théorème d'impossibilité d'Arrow, les applications réelles et les alternatives électorales (Borda, Jugement Majoritaire, IRV).
